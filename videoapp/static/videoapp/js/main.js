    ...
    recognizer.recognized = async (s, e) => {
        if (e.result.reason === SpeechSDK.ResultReason.RecognizedSpeech) {
            ...
            if (recognizedText !== lastRecognizedText) {
                ...
                const translatedText = recognizedText;
...
async function synthesizeSpeech1(text) {
    ...
    utterance.lang = 'en-GB' //;targetLanguage;  // Peut aussi être 'en-US' si vous voulez l'accent américain
...
function synthesizeSpeech(text) {
    ...
    if ('speechSynthesis' in window) {
      const utterance = new SpeechSynthesisUtterance(text);
      utterance.lang = language; // Définir la langue, par exemple : 'fr-FR', 'en-US', etc.
  
      // Obtenir les voix disponibles et choisir automatiquement
      const voices = window.speechSynthesis.getVoices();
      if (voices.length > 0) {
        // Rechercher une voix qui correspond à la langue
        let selectedVoice = voices.find(voice => voice.lang.startsWith(language));
  
        // Si aucune voix ne correspond exactement, prendre la première voix disponible
        if (!selectedVoice) {
  ...
  playTextWithAutoVoice("Bonjour, comment ça va ?", "fr-FR");
...
voiceSocket.onmessage = async function (e) {
    ...
    if (data.username !== currentUsername && data.transcript) {
        ...
        if (!audioPlaying){
            console.log("Console1", data.transcript);
            const text_synt = await translateText(data.transcript);
            console.log("Console2", text_synt);
            synthesizeSpeech(text_synt);
        }
